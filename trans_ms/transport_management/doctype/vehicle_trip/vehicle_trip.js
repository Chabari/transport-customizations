...
frappe.ui.form.on('Vehicle Trip', {
    ...
    refresh: function (frm) {
        ...
        if (frm.doc.trip_completed == 1 && frm.doc.custom_transport_type == "Cross Border" && frm.doc.custom_has_local_trip == 0) {
            frm.add_custom_button(__("Load Local Trip"), function () {
                
                var d = new frappe.ui.Dialog({
                    title: __('Enter Loading Quantity'),
                    fields: [
                        {
                            "label" : "Quantity",
                            "fieldname": "quantity",
                    ...
                    primary_action: function() {
                        var data = d.get_values();
                        if(data.quantity < 1){
                                frappe.msgprint(__("Enter quantity"));
                            return
                        }
                        frappe.call({
                            method: "trans_ms.transport_management.doctype.vehicle_trip.vehicle_trip.create_local_trip",
                            args: {
                                name: frm.doc.name,
                                quantity: data.quantity,
                            },
                            callback: function(r) {                                
                                d.hide();
                                if(r.message == true){
							        frappe.set_route('Form', r.data.doctype, r.data.name);
                    ...
                    primary_action_label: __('Start Trip')
                });
                d.show();
