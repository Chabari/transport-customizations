        ...
        trip.update(
            {
                ...
                "custom_shipping_address": doc.custom_shipping_address,
                "custom_loaded_quantity": doc.net_weight,
...
def get_trip_total(name):
    doc = frappe.get_doc("Vehicle Trip", name)
    if doc.reference_doctype:
        assignment = frappe.get_doc(doc.reference_doctype, doc.reference_docname)
        return assignment.rate * doc.custom_loaded_quantity
    elif doc.custom_parent_trip:
        ptrip = frappe.get_doc("Vehicle Trip", doc.custom_parent_trip)
        if ptrip.reference_doctype:
            assignment = frappe.get_doc(ptrip.reference_doctype, ptrip.reference_docname)
...
@frappe.whitelist(allow_guest=True)
def create_local_trip(**args):
    try:
        doc = frappe.get_doc("Vehicle Trip", args.get('name'))
        trip = frappe.new_doc("Vehicle Trip")
        trip.update(
            {
                "status": "En Route",
                "main_cargo_location_country": doc.main_cargo_location_country,
                "main_cargo_location_city": doc.main_cargo_location_city,
                "main_cargo_destination_country": doc.main_cargo_destination_country,
                "main_cargo_destination_city": doc.main_cargo_destination_city,
                "custom_shipping_address": doc.custom_shipping_address,
                "custom_loaded_quantity": args.get('quantity'),
                "custom_transport_type": "Internal",
                "custom_transport_order": doc.custom_transport_order,
                "custom_parent_trip": doc.name,
                "main_cargo_category": None,
                "vehicle_plate_number": doc.vehicle_plate_number,
                "customer": doc.customer,
                "main_route": doc.main_route,
                "vehicle": doc.vehicle,
                "trailer": doc.trailer,
        ...
        frappe.set_value("Vehicle", trip.vehicle, "status", "In Trip")
        frappe.set_value("Vehicle", trip.vehicle, "current_trip", trip.name)


        doc.custom_has_local_trip = 1
        doc.db_update()
        frappe.response.message = True
        frappe.response.data = trip
    except Exception as e:
        frappe.log_error(frappe.get_traceback(), str(e))
        frappe.response.message = str(e)
